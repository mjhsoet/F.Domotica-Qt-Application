...
#if defined WIN32 || defined _WIN32 || defined _WIN32_ || defined __WIN32__ \
	...
	int length)
{
    ...
    success = WriteFile( serial->hCom, buffer, (DWORD)length, &dwWrote, NULL);
	#ifdef XBEE_SERIAL_VERBOSE
...
int xbee_ser_read( xbee_serial_t *serial, void FAR *buffer, int bufsize)
{
    ...
    success = ReadFile( serial->hCom, buffer, (DWORD)bufsize, &dwRead, NULL);
	#ifdef XBEE_SERIAL_VERBOSE
		if (! success)
		{
...
int xbee_ser_open( xbee_serial_t *serial, uint32_t baudrate)
{
    ...
    wchar_t         wideBuffer[sizeof("\\\\.\\COM9999")];
	HANDLE			hCom;
	...
	else
	{
        ...
        mbstowcs(wideBuffer,buffer,strlen(buffer)+1);
        hCom = CreateFile( wideBuffer, GENERIC_READ | GENERIC_WRITE,
